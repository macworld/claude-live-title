...
Before the AI text reaches Haiku, a sanitize pass strips fenced code blocks, inline backticks, Python/JS stack frames, and `$` / `>` shell prompt lines. If under 30 bytes of substance remain, the `STATE:` line is dropped entirely — this prevents titles from latching onto `Traceback` lines or shell-output fragments when Claude's last paragraph is mostly code or errors.
