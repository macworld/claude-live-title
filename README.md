...
Titles are generated by calling a lightweight model (Haiku by default) with a sample of your conversation — the earliest few and most recent few user messages (8 total by default, adjustable via `contextMessages`) — then written to the session transcript as a `custom-title` entry.
...
```jsonc
{
  ...
  // Number of user messages sampled for title generation (default: 8)
  // Samples the earliest ~3/8 and most recent ~5/8 of user messages
  "contextMessages": 8,
