...
AI 文字在进 Haiku 之前会先过一道清洗：剥离 fenced code block、inline backtick、Python/JS 堆栈帧、以及 `$` / `>` 开头的 shell 提示符行。如果剩下的实质内容不足 30 字节，整条 `STATE:` 就会被丢弃——这样可以避免 Claude 上一段恰好是 `Traceback` 或命令输出时，标题跟着抄进一串无意义的报错片段。
